            ...
            if crit_vecs > 0 and len(C_list) > 1:
                ...
                for refine_iter in range(10):
                    ...
                    for j in range(1, len(C_list)):
                        ...
                        occ=np.bincount(B_ext,minlength=C_ext.shape[0])
                        if (occ<=1).sum()/occ.size>0.25:
                            log.warning(f"{name}:1/4 of the centroids are almost unused; consider smaller lambda or more codebooks")
                    ...
                    if cur_error < prev_error: # improvement ⇒ keep
                        best_C_list = copy.deepcopy(C_list)
                        best_B_list = copy.deepcopy(B_list)
                        prev_error = cur_error
                    else:
                        C_list = best_C_list  ####instead of like above, direct assignment in change.new
                        B_list = best_B_list
                        break
